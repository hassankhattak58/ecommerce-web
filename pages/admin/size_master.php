...
<head>
    ...
    if (isset($_POST['cat_submit'])) {
        $size_number = safe_value($con, $_POST['color_name']);
        $query_add = mysqli_query($con, "INSERT INTO `siZe_master` (`id`, `size_number`) VALUES (NULL, '$size_number')");
    }
...
<body>
    ...
    <div class="flex maindashboard">
        ...
        <div class="h-auto justify-center text-center product_list_container" id="products">
            ...
            <div class="w-full bg-orange-600 h-14 py-3">
                <h1 class="text-3xl">Size Master</h1>
            </div>
            <div class="flex flex-row">
                <table class='text-left border-2 bg-white w-[30%] text-xl'>
                    ...
                    <?php
                    $sqlquery = "SELECT * FROM `size_master`";
                    ...
                    <td class='py-2 border-[#000] border-2'>$row[size_number] <span style='float:right; padding: 0px 10px; cursor:pointer;'><button onclick='confirmdelete(this)' id='$row[id]' style='text-decoration:underline;'>Delete</button></span></td>
                    </tr>
                ");
                    }
                    ?>
                ...
                <div class="w-[70%] h-auto" style="padding: 18px 70px;">
                    <form method="POST" action="size_master.php">
                        <span class="mt-4">
                            <label for="catagory_name" class="text-2xl flex flex-col text-left leading-10">Size Name
                                <input type="number" name="color_name" id="cat_title" class="w-96 h-10 px-5 py-3 pl-3" required>
                            </label>
                        </span>
    ...
    <script>
        ...
        function delete_color(button) {
            window.location = "delete_color.php?master=size&id=" + confirm_id;
        }
    </script>
